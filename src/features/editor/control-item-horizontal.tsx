...
import { AdvancedAudioEffects } from "./control-item/advanced-audio-effects";
...
const ColorPickerControl = ({
	...
	useEffect(() => {
		...
		if (trackItem?.type === "text") {
			...
			currentColor =
				trackItem.details?.appearedColor || CAPTION_DEFAULTS.appearedColor;
...
const CaptionAppearedColorPickerControl = ({
	...
	const [localValue, setLocalValue] = useState<string>(
		CAPTION_DEFAULTS.appearedColor,
	);
	const isLargeScreen = useIsLargeScreen();
	const { setControItemDrawerOpen } = useLayoutStore();
...
const CaptionActiveColorPickerControl = ({
	...
	useEffect(() => {
		...
		const currentActiveColor =
			trackItem?.details?.activeColor || CAPTION_DEFAULTS.activeColor;
...
const CaptionActiveFillColorPickerControl = ({
	...
	const [localValue, setLocalValue] = useState<string>(
		CAPTION_DEFAULTS.activeFillColor,
	);
	const isLargeScreen = useIsLargeScreen();
	const { setControItemDrawerOpen } = useLayoutStore();
	...
	useEffect(() => {
		...
		const currentActiveFillColor =
			trackItem?.details?.activeFillColor || CAPTION_DEFAULTS.activeFillColor;
...
const ItemAudio = ({
	...
	<ItemGroup
		items={[
			...
			{ icon: Icons.audio, label: "Audio Effects", id: "audioEffects" },
		]}
		handleMenuItemClick={handleMenuItemClick}
	/>
);
