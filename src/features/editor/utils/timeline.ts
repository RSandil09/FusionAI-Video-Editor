...
export function getPreviousZoomLevel(
	...
	if (!currentZoom || typeof currentZoom.zoom !== "number") {
		return TIMELINE_ZOOM_LEVELS[0];
	}
...
export function getZoomByIndex(index: number) {
	const safeIndex = Math.max(
		0,
		Math.min(index, TIMELINE_ZOOM_LEVELS.length - 1),
	);
	return TIMELINE_ZOOM_LEVELS[safeIndex] || TIMELINE_ZOOM_LEVELS[0];
}
export function getNextZoomLevel(
	currentZoom: ITimelineScaleState,
): ITimelineScaleState {
	if (!currentZoom || typeof currentZoom.zoom !== "number") {
		return TIMELINE_ZOOM_LEVELS[TIMELINE_ZOOM_LEVELS.length - 1];
	}
