...
export class VideoItem extends BaseItem {
  ...
  private buildFallbackOffscreen() {
    ...
    if (!fallback) return;
    const maxW = 12000;
    const totalW = Math.min(maxW, this.thumbnailsPerSegment * this.thumbnailWidth * 10);
    ...
    for (let i = 0; i < total; i++) {
      ctx.drawImage(fallback, i * this.thumbnailWidth, 0, this.thumbnailWidth, this.thumbnailHeight);
    }
    this.fallbackOffscreen = c;
    this.fallbackCtx = ctx;
  }

  private calculateFilmstripDimensions(segmentIndex: number, widthOnScreen: number) {
    ...
    const rightRemaining = totalW - widthOnScreen - leftBacklog - filmstripOffset;
  ...
  private calcVisibleWidth() {
    ...
    const cutRight = Math.max(cw - (this.width + this.left + this.scrollLeft), 0);
  ...
  onScrollChange(scrollLeft: number, force = false) {
    ...
    const seg = calculateOffscreenSegments(offW, trimFromSize, this.segmentSize);
