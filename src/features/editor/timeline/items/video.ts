...
class Video extends Trimmable {
	...
	private async loadFallbackThumbnail() {
		// Try previewUrl first, then fall back to extracting from video
		const fallbackThumbnail = this.previewUrl || this.src;
		if (!fallbackThumbnail) return;

		// If it's a video URL (no image extension), extract frame from video
		const isVideoUrl =
			this.src &&
			(this.src.includes(".mp4") ||
				this.src.includes(".webm") ||
				this.src.includes(".mov") ||
				!this.previewUrl);

		if (isVideoUrl && !this.previewUrl) {
		...
		return new Promise<void>((resolve) => {
			...
			img.onerror = () => {
				// If image fails to load, try extracting from video
				this.extractVideoThumbnail().then(resolve);
			};
		});
	}
