...
export const useStateManagerEvents = (stateManager: StateManager) => {
	...
	const handleAddRemoveItems = useCallback(() => {
		...
		console.log("✅ handleAddRemoveItems triggered:", {
			...
			itemCount: Object.keys(mergedTrackItemsDeatilsMap).length,
	...
	useEffect(() => {
		console.log("🔌 useStateManagerEvents mounting...", stateManager);
		...
		if (!registry) {
			console.warn("⚠️ Failed to get registry");
			return;
		}

		const hookId = "useStateManagerEvents";

		// Prevent duplicate subscriptions - but log when this happens
		if (registry.has(hookId)) {
			console.log("⏭️ Skipping subscription setup - already registered");
			return;
		}

		console.log("✅ Setting up state manager subscriptions");
		registry.add(hookId);
